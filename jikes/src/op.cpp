...
signed char get_i1(Tuple<u1>& code, int pc) {
    return code[pc];
}
short get_i2(Tuple<u1>& code, int pc) {
    return  code[pc] << 8 | code[pc+1];
}
int get_i4(Tuple<u1>& code, int pc) {
    return  code[pc] << 24 | code[pc+1] << 16 | code[pc+2] << 8 | code[pc+3];
}
unsigned int get_u1(Tuple<u1>& code, int pc) {
    unsigned val;
    val = code[pc];
    return val;
}
unsigned int get_u2(Tuple<u1>& code, int pc) {
    unsigned int val;
    val = (code[pc] << 8 | code[pc+1]);
    return val;
}
unsigned int get_u4(Tuple<u1>& code, int pc) {
    unsigned int val;
    val = (code[pc] << 24 | code[pc+1] << 16 | code[pc+2] << 8 | code[pc+3]);
    return val;
}
extern void const_string(Tuple<cp_info *>& constant_pool, int indx);
void opline(Tuple<cp_info *>& constant_pool,char * hdr, int pc, int opc, char *name,
            char* args, char* desc,
